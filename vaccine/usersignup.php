...
session_start();
	...
	if($_SERVER['REQUEST_METHOD'] == "POST")
	{
		...
		if(!empty($fullname) && !empty($user_name) && !is_numeric($user_name) && !empty($address) && !empty($age) && !empty($contact_no) && !empty($password) && is_numeric($age))
		{

			
			$user_id = random_num(20);
			$query = "insert into client (user_id,fullname,user_name,address,age,contact_no,password)
		   			  values ('$user_id','$fullname','$user_name','$address','$age','$contact_no','$password')";
  ...
  <body>
    <div class="container">
      <form method="post">
        ...
        <label for="Contact">𝐂𝐨𝐧𝐭𝐚𝐜𝐭</label>
        <br>
        <input type="text" id="cont" name="contact_no" placeholder="Contact:">
        <br>

        <label for="Age">𝐀𝐠𝐞</label>
        <br>
        ...
        <a href="userlogin.php" class="login">LOGIN INSTEAD</a>
        <br>
      </form>
    </div>
  </body>
