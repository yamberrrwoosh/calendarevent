...
session_start();
  ...
  if(isset($_POST['Change']))
  {
    ...
    $query = mysqli_query($con,"SELECT pword FROM admins
    WHERE admin_id = '$id_num' AND pword ='$op'");
    $num = mysqli_fetch_array($query);

    if($num>0)
    {
      $sql = mysqli_query($con,"UPDATE admins SET pword ='$np' WHERE admin_id = '$id_num'");
...
function myFunction() {
    ...
    <body>
      ...
      <img src="https://media2.giphy.com/media/WUyeycaNusqDWv7KUz/giphy.gif?cid=6c09b9523c8491c796db8eb4a5354f50721bd859c8cc4124&rid=giphy.gif&ct=s" style="width:30%;">
        ...
        <input type="password" id="pword" name="op" placeholder="Enter Old Password">
        <br>
        <br>

        <input type="password" id="pword" name="np" placeholder="Enter New Password">
        <br>
        <br>

        <input type="password" id="pword" name="c_np" placeholder="Re-enter New Password">
        <br>
        <br>
...
<!-- dito gagamit tayo logical AND para sa new password, if new password == re enter new password magiging TRUE then change password na -->
        ...
        <a href="adminsettings.php" class="cancel">Cancel</a>
      </form>
      </div>
